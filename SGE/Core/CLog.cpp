...
void CLog::Initialize(const char* pFilename)
{
	...
	if (nullptr == pFile)
	{
		MessageBoxA(nullptr, "Error: Cannot open file for error log", "CLog", MB_OK | MB_ICONERROR);
		return;
	}
...
void CLog::Write(ELogMessageType eLogMessageType, const char* pMessage, va_list args)
{
	...
	if (!mInitialized)
	{
		MessageBoxA(nullptr, "Error: Log must be initialized first before writing any messages.", "CLog", MB_OK | MB_ICONERROR);
		return;
	}
