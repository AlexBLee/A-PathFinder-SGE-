...
{
	const float kCosAngle = static_cast<float>(cos(rad));
	const float kSinAngle = static_cast<float>(sin(rad));
...
SVector2 SVector2::operator*(const SVector2& f) const
{
	return SVector2(x * f.x, y * f.y);
}
...
{
	const float c = static_cast<float>(cos(rad));
	const float s = static_cast<float>(sin(rad));
...
float Distance(const SVector2& v0, const SVector2& v1)
{
	...
	return static_cast<float>(sqrt(fDiffX * fDiffX + fDiffY * fDiffY));
}
